...
export const QuotationPDFEnhanced = ({ quotation, mode = 'plain-model' }: QuotationPDFProps) => {
      ...
      return (
        <View style={{ marginBottom: 20 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View>
              <Text style={{ fontSize: 28, fontWeight: 'bold' }}>QUOTATION</Text>
            </View>
            <View>
              <Text style={styles.quotationNumber}>
                #{quotation.quotationNumber || quotation.id.substring(0, 8).toUpperCase()}
              </Text>
              {quotation.salesExecutiveId && (
                <Text style={styles.salesExecutive}>Sales Exec: {quotation.salesExecutiveId}</Text>
              )}
    ...
    if (mode === 'plain') {
      ...
      return (
        <View style={styles.plainHeader}>
          <Text style={styles.plainTitle}>QUOTATION</Text>
          ...
          <View style={{ marginTop: 15, flexDirection: 'row', justifyContent: 'space-between' }}>
            <View>
              <Text style={{ fontSize: 12, fontWeight: 'bold' }}>
                Quote #: {quotation.quotationNumber || quotation.id.substring(0, 8).toUpperCase()}
              </Text>
              {quotation.salesExecutiveId && (
                <Text style={{ fontSize: 9, marginTop: 3 }}>Sales Exec: {quotation.salesExecutiveId}</Text>
              )}
            </View>
            <Text style={{ fontSize: 11, fontWeight: 'bold' }}>{quotation.status}</Text>
          </View>
        </View>
      )
    }
    ...
    <Document>
      <Page size="A4" style={getPageStyle()}>
        ...
        {quotation.validUntil && (
          <View style={isPlainMode ? styles.validUntilPlain : styles.validUntil}>
        ...
        <View style={isPlainMode ? styles.infoCardPlain : styles.infoCard}>
          <View style={styles.row}>
            ...
            <Text style={styles.value}>{format(new Date(quotation.createdAt), 'PP')}</Text>
          </View>
          {quotation.validUntil && (
            <View style={styles.row}>
              <Text style={styles.label}>Valid Until:</Text>
              <Text style={styles.value}>{format(new Date(quotation.validUntil), 'PP')}</Text>
            </View>
          )}
        </View>
        ...
        <View style={isPlainMode ? styles.billToSectionPlain : styles.billToSection}>
          <Text style={styles.billToHeader}>{quotation.client ? 'Bill To:' : 'Quote For:'}</Text>
          <Text style={styles.billToText}>{clientName}</Text>
          <Text style={styles.billToText}>Phone: {clientPhone}</Text>
          {clientEmail && <Text style={styles.billToText}>Email: {clientEmail}</Text>}
          {quotation.site && (
            <View style={{ marginTop: 5 }}>
              <Text style={styles.billToText}>Site: {quotation.site.name}</Text>
              {quotation.site.address && <Text style={styles.billToText}>Address: {quotation.site.address}</Text>}
            </View>
          )}
        </View>
        ...
        <View style={styles.totalsSection}>
          <View style={isPlainMode ? styles.grandTotalPlain : styles.grandTotal}>
            <Text style={isPlainMode ? styles.grandTotalLabelPlain : styles.grandTotalLabel}>Grand Total:</Text>
