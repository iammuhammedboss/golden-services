...
// GET /api/masters/equipment - List all equipment (including deleted if requested)
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('GET /api/masters/equipment error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}

// POST /api/masters/equipment - Create new equipment (OWNER only)
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('POST /api/masters/equipment error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
