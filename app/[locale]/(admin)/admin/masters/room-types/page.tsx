...
export default function RoomTypesPage() {
  ...
  const [editingItem, setEditingItem] = useState<RoomTypeMaster | null>(null)
  ...
  const fetchRoomTypes = async () => {
    try {
      const response = await fetch('/api/masters/room-types')
      if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingItem
        ? `/api/masters/room-types/${editingItem.id}`
  ...
  const handleEdit = (item: RoomTypeMaster) => {
    setEditingItem(item)
    setFormData({
      name: item.name,
  ...
  const handleDelete = async (itemId: string) => {
    ...
    try {
      const response = await fetch(`/api/masters/room-types/${itemId}`, {
        method: 'DELETE',
      })
      ...
      if (response.ok) {
        ...
        const error = await response.json()
        alert(error.error || 'Failed to delete room type')
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage room types and spaces</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            <form onSubmit={handleSubmit}>
              <DialogHeader>
                <DialogTitle>{editingItem ? `${tc('edit')} Room Type` : `${tc('add')} Room Type`}</DialogTitle>
                <DialogDescription>
                  {editingItem
                    ? `${tc('update')} Room Type`
                    : `${tc('create')} Room Type`}
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="e.g., HALL, BEDROOM, KITCHEN"
                    required
                  />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor="sortOrder">{tc('sortOrder')}</Label>
                  <Input
                    id="sortOrder"
                    type="number"
      ...
      <Card>
        <CardHeader>
          <CardTitle>{tc('all')} Room Types</CardTitle>
          <CardDescription>{tc('total')}: {roomTypes.length}</CardDescription>
        </CardHeader>
