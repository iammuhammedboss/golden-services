...
export default function PaymentMethodsPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button onClick={() => handleDialogClose()}>
              ...
              {tc('add')} Payment Method
            </Button>
          </DialogTrigger>
          <DialogContent>
            <form onSubmit={handleSubmit}>
              <DialogHeader>
                <DialogTitle>{editingItem ? `${tc('edit')} Payment Method` : `${tc('add')} Payment Method`}</DialogTitle>
                <DialogDescription>
                  {editingItem
                    ? `${tc('update')} Payment Method`
                    : `${tc('create')} Payment Method`}
                </DialogDescription>
              </DialogHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{tc('all')} Payment Methods</CardTitle>
          <CardDescription>{tc('total')}: {paymentMethods.length}</CardDescription>
        </CardHeader>
