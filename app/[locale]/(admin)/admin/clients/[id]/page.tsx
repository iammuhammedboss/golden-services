...
export default function ClientDetailsPage() {
  ...
  // Action tiles
  const actionTiles = [
    { label: tc('edit'), href: `/${locale}/admin/clients/${id}/edit`, color: 'from-blue-500 to-blue-600', icon: <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg> },
    { label: t('ledger'), href: `/${locale}/admin/clients/${id}/ledger`, color: 'from-emerald-500 to-emerald-600', icon: <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z" /></svg> },
    { label: t('quotes'), href: `/${locale}/admin/quotations?clientId=${id}`, color: 'from-orange-500 to-orange-600', icon: <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> },
    { label: t('newQuote'), href: `/${locale}/admin/quotations/new?clientId=${id}`, color: 'from-amber-500 to-amber-600', icon: <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4v16m8-8H4" /></svg> },
    { label: t('newJob'), href: `/${locale}/admin/jobs/new?clientId=${id}`, color: 'from-cyan-500 to-cyan-600', icon: <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4v16m8-8H4" /></svg> },
    { label: t('invoice'), href: `/${locale}/admin/invoices?clientId=${id}`, color: 'from-pink-500 to-pink-600', icon: <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg> },
    { label: t('payments'), href: `/${locale}/admin/payments?clientId=${id}`, color: 'from-green-500 to-green-600', icon: <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" /></svg> },
    { label: t('siteVisit'), href: `/${locale}/admin/site-visits?clientId=${id}`, color: 'from-violet-500 to-violet-600', icon: <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg> },
  ]
    ...
    <div className="mx-auto max-w-2xl space-y-5 pb-8">
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          ...
          {client.email ? (
            ...
            <div className="flex items-center justify-center rounded-xl border border-gray-100 bg-gray-50 py-2.5 text-xs text-gray-400">{t('noEmail')}</div>
          )}
        </div>
      </div>
      ...
      <div className="flex rounded-xl border border-gray-100 bg-gray-50 p-1">
        {(['info', 'jobs', 'payments'] as const).map((tab) => (
          ...
          >
            {tab === 'info' ? t('info') : tab === 'jobs' ? `${t('jobs')} (${jobs.length})` : `${t('payments')} (${payments.length})`}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'info' && (
        <div className="space-y-3">
          <InfoSection title={t('contactInfo')}>
            <InfoRow label="Phone" value={client.phone} />
            {client.alternatePhone && <InfoRow label="Alt Phone" value={client.alternatePhone} />}
            {client.whatsapp && client.whatsapp !== client.phone && <InfoRow label={tc('whatsapp')} value={client.whatsapp} />}
            {client.email && <InfoRow label={tc('email')} value={client.email} />}
          </InfoSection>
          {client.notes && (
          ...
          <InfoSection title={t('meta')}>
            <InfoRow label={tc('source')} value={enumToReadable(client.source)} />
            <InfoRow label={tc('created')} value={formatDate(client.createdAt, 'PPP')} />
          </InfoSection>
      ...
      {activeTab === 'payments' && (
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-green-100 bg-green-50 p-3 text-center">
            <p className="text-[10px] font-medium uppercase tracking-wider text-gray-400">{t('totalPaid')}</p>
          ...
          {showPaymentForm && (
            <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => { setShowPaymentForm(false); setPaymentError('') }} className="text-xs text-gray-400 hover:text-gray-600">{tc('cancel')}</button>
              </div>
          ...
          {payments.length > 0 ? (
            ...
            <div className="py-8 text-center">
              <p className="text-sm text-gray-400">{t('noPaymentsRecorded')}</p>
            </div>
          )}
        </div>
      )}
