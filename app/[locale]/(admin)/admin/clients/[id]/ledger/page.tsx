    ...
    <div className="mx-auto max-w-2xl space-y-5 pb-8">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="rounded-xl border border-red-100 bg-red-50 p-3 text-center">
          <p className="text-[10px] font-medium uppercase tracking-wider text-gray-400">{t('invoiced')}</p>
          <p className="mt-0.5 text-base font-bold text-red-600">{totalDebit.toFixed(2)}</p>
          <p className="text-[10px] text-gray-400">{tc('omr')}</p>
        </div>
        <div className="rounded-xl border border-green-100 bg-green-50 p-3 text-center">
          <p className="text-[10px] font-medium uppercase tracking-wider text-gray-400">{t('paid')}</p>
        ...
        <div className={`rounded-xl border p-3 text-center ${
          ...
          <p className="text-[10px] font-medium uppercase tracking-wider text-gray-400">{t('balance')}</p>
      ...
      <div>
        <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-gray-400">
          {t('transactions')} ({ledgerEntries.length})
        </h3>
        {ledgerEntries.length > 0 ? (
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-gray-400">
                      {tc('bal')}: {entry.balance.toFixed(3)}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="py-12 text-center">
            ...
            <p className="mt-2 text-sm text-gray-400">{t('noTransactions')}</p>
          </div>
        )}
      </div>
    </div>
